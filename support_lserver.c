...
struct lsstruct *init_ls(void) {
  ...
  lssp=(struct lsstruct *)malloc(sizeof(struct lsstruct));
  lssp->lserver=NULL;
  lssp->domain=NULL;
  lssp->next=NULL;
  return lssp;
...
int create_ls(struct main_args *margs) {
  ...
  np=margs->llist;
  if (margs->debug)
    fprintf(stderr, "%s| %s: ldap server list %s\n",LogTime(), PROGRAM,margs->llist?margs->llist:"NULL");
  dp=NULL;

  if (!p) {
    if (margs->debug)
      fprintf(stderr, "%s| %s: No ldap servers defined.\n",LogTime(), PROGRAM);
    return(0);
  }
  while (*p) { /* loop over group list */
	...
	if (margs->debug)
	  fprintf(stderr, "%s| %s: No ldap servers defined for domain %s\n",LogTime(), PROGRAM,p);
	return(1);
      }
      *p = '\0';
      p++; 
      lssp=init_ls();
      lssp->lserver=strdup(np);
	...
	if (lsspn) /* Have already an existing structure */
      ...
      if (!lssp->domain || !strcmp(lssp->domain,"")) {
        if (margs->debug)
          fprintf(stderr, "%s| %s: No domain defined for ldap server %s\n",LogTime(), PROGRAM,lssp->lserver);
        return(1);
      }
      if (margs->debug) 
	fprintf(stderr, "%s| %s: ldap server %s  Domain %s\n",LogTime(), PROGRAM,lssp->lserver,lssp->domain);
    } else 
      p++;
  }
  if (p == np) { /* empty group name not allowed */
