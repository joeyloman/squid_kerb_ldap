...
struct gdstruct *init_gd(void) {
  ...
  gdsp=(struct gdstruct *)malloc(sizeof(struct gdstruct));
  gdsp->group=NULL;
  gdsp->domain=NULL;
  gdsp->next=NULL;
  return gdsp;
...
int create_gd(struct main_args *margs) {
  ...
  if (hp1) {
    if (hp2) {
      if (up) {
        p=malloc(strlen(up)+strlen(hp1)+strlen(hp2)+2);
        strcpy(p,up);
        strcat(p,":");
        strcat(p,hp1);
        strcat(p,":");
        ...
        p=malloc(strlen(hp1)+strlen(hp2)+1);
        strcpy(p,hp1);
        strcat(p,":");
        strcat(p,hp2);
      }
      ...
      if (up) {
        p=malloc(strlen(up)+strlen(hp1)+1);
        strcpy(p,up);
        strcat(p,":");
        strcat(p,hp1);
      } else
    ...
    if (hp2) {
      if (up) {
        p=malloc(strlen(up)+strlen(hp2)+1);
        strcpy(p,up);
        strcat(p,":");
        strcat(p,hp2);
      } else 
